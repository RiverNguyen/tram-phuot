...
const TabButtons = ({ kindTerms, activeTab, onTabChange }: TabButtonsProps) => {
  return (
    <div className='xsm:mx-auto inline-flex items-center justify-end gap-[0.5625rem] relative z-[1]'>
      {kindTerms.map((taxonomy, index: number) => {
        ...
        return (
          <button
            ...
            className={`flex h-[2.45925rem] sm:px-[1.875rem] justify-center items-center gap-[0.5625rem] rounded-[0.625rem] cursor-pointer transition-all ${isActive
          ...
          >
            ...
            />
            <span
              className={`xsm:text-[0.75rem] xsm:leading-[0.9rem] whitespace-nowrap font-montserrat text-[0.875rem] font-semibold leading-[1.05rem] uppercase ${isActive
                  ? 'text-[#F9EAD5]'
