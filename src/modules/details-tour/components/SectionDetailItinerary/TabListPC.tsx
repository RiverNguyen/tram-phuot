...
interface TabListPCProps {
  items: NormalizedDetailsTourDetailItineraryType[]
}

export default function TabListPC({ items }: TabListPCProps) {
  return (
    <TabsList className='hidden_scroll xsm:space-x-1.5 xsm:hidden flex h-auto w-full justify-start overflow-x-auto rounded-[2.75rem] bg-transparent p-0'>
      <Swiper
        slidesPerView='auto'
        spaceBetween={convertRemToPx(0.5) || 8}
        className='w-full'
        grabCursor
        freeMode={true}
      >
