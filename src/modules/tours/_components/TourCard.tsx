...
export default function TourCard({ tour, className }: { tour: ITour; className?: string }) {
  ...
  return (
    <Link
      href={href}
      className={cn(
        'block xsm:h-[31.03531rem] xsm:rounded-[0.56044rem] group relative w-full h-[22.6875rem] rounded-[0.5rem] bg-[#FFFCE3] overflow-hidden',
        className,
      )}
    >
      {/* background */}
      ...
      {Array.isArray(tour?.taxonomies['tour-type']) && tour.taxonomies['tour-type'].length > 0 && (
        <div className='xsm:px-[0.84069rem] xsm:py-[0.70056rem] absolute top-0 left-0 w-full inline-flex px-[0.75rem] py-[0.625rem]'>
          <div className='xsm:h-[1.75144rem] xsm:px-[0.99013rem] xsm:rounded-tl-[0.84069rem] xsm:rounded-br-[0.84069rem] flex h-[1.5625rem] px-[0.88331rem] justify-center items-center rounded-tl-[0.75rem] rounded-br-[0.75rem] bg-[#2BAB7D]'>
            <span className='xsm:text-[0.625rem] xsm:leading-[1.75rem] xsm:tracking-[-0.0175rem] line-clamp-1 text-white font-phu-du text-[0.75rem] font-medium leading-[1.5rem] tracking-[-0.015rem]'>
      ...
      {/* content */}
      <div className='xsm:min-h-auto xsm:pt-[3.50288rem] xsm:px-[0.84069rem] xsm:pb-[1rem] xsm:items-center absolute bottom-0 left-0 flex w-full min-h-[12.375rem] pt-[3.125rem] px-[0.75rem] pb-[1.1875rem] justify-between items-end bg-[linear-gradient(180deg,rgba(6,42,25,0.00)_0%,rgba(6,42,25,0.58)_24.91%,#062A19_100%)]'>
        <div className='xsm:gap-[0.91075rem] w-full h-full flex flex-col items-start gap-[0.8125rem]'>
          {/* content top */}
          <div className='xsm:gap-[0.84069rem] flex flex-col justify-center gap-[0.75rem] self-stretch'>
            ...
            {/* title */}
            <h3 className='xsm:text-[1rem] xsm:leading-[1.25rem] xsm:tracking-normal line-clamp-2 text-white font-phu-du text-[1rem] font-medium leading-[1.3rem] uppercase'>
              {tour?.title}
            </h3>
          </div>
          ...
          <div className='xsm:justify-between xsm:items-end w-full h-full flex items-center gap-[0.625rem]'>
            <div className='xsm:gap-[0.5rem] xsm:shrink-0 flex-1 flex flex-col items-start gap-[0.9375rem]'>
              {/* line */}
              <div className='xsm:h-[0.07006rem] w-full h-[0.0625rem] bg-[rgba(255,255,255,0.1)]' />
              <div className='w-full flex justify-between items-center'>
                <div className='xsm:gap-[0.28025rem] flex items-center gap-[0.25rem]'>
                  ...
                  <span className='xsm:text-[0.75rem] xsm:leading-[1.2rem] xsm:tracking-[-0.01rem] text-white font-montserrat text-[0.875rem] font-semibold leading-[1.4rem] tracking-[-0.00875rem] opacity-[0.48]'>
                    {Array.isArray(tour?.taxonomies?.locations)
                      ? tour?.taxonomies?.locations[0]?.name
                      : ''}
                  </span>
                </div>
                <p className='xsm:leading-[1.1375rem] font-phu-du font-medium leading-[1.3rem] uppercase'>
                  <span className='xsm:text-[0.875rem] xsm:leading-[1.1375rem] text-[#FFC542] text-[1rem]'>
            ...
            <div className='xsm:size-[2rem] xsm:py-[0.91188rem] xsm:shrink-0 xsm:rounded-[0.684rem] flex size-[2.25rem] py-[0.75rem] items-center justify-center rounded-[0.5625rem] bg-[#F56E0A]'>
